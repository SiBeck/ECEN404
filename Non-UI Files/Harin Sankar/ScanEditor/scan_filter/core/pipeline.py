...
from ..io.mrd_reader import MRDSolutionsReader
...
class ScanFilterPipeline:
    def __init__(
        ...
        mrd_reader: Optional[MRDSolutionsReader] = None,
    ) -> None:
        self.config = config
        ...
        io_cfg = config.io
        zero_pad_shape = None
        fe_pad = getattr(io_cfg, "mrd_zero_pad_fe", None)
        pe_pad = getattr(io_cfg, "mrd_zero_pad_pe", None)
        if fe_pad is not None and pe_pad is not None:
            zero_pad_shape = (int(fe_pad), int(pe_pad))
        self.mrd_reader = mrd_reader or MRDSolutionsReader(
            frame_period_ms=getattr(io_cfg, "mrd_frame_period_ms", 40.0),
            start_time_ms=getattr(io_cfg, "mrd_start_time_ms", 0.0),
            apply_hamming_window=getattr(io_cfg, "mrd_apply_hamming_window", False),
    ...
    def run(self, cardiogram_csv: str | Path, dicom_folder: str | Path, output_dir: Optional[str | Path] = None) -> PipelineArtifacts:
        ...
        self.logger.info("Loading MRI data from %s", dicom_folder)
        frames = self._load_mri_frames(dicom_folder)
    ...
    def _load_mri_frames(self, source: str | Path) -> MRIFrameSeries:
        path = Path(source)
        if path.is_file() and path.suffix.lower() == ".mrd":
            self.logger.info("Detected MR Solutions MRD input")
            return self.mrd_reader.read(path)
        return self.dicom_reader.read(path)
